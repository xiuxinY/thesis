...
if __name__ == '__main__':
    ...
    single_result = pd.DataFrame(
        np.zeros((len(nodes), len(keys))), columns=keys)

    for key in keys:
        for i in range(len(nodes)):
            print('Training ' + key + ' with ' + str(nodes[i]) + ' nodes')
            x_train, y_train = getData(performance_name=key)
            model = getModel(nodes_num=nodes[i])
            model_checkpoint_callback = create_checkpoint_callback(
                performance_name=key, node_num=nodes[i])
            acc, history = Model_Train(model, x_train, y_train)
            history_save(performance_name=key,
                         node_num=nodes[i], history=history)
            result_save(performance_name=key, node_num=nodes[i], acc=acc)
            single_result.loc[i, key] = acc

    nodes_df = pd.DataFrame(nodes, columns=['Nodes'])
    results = pd.concat([nodes_df, single_result], axis=1)
    writer = pd.ExcelWriter("scalar_result_nodes.xlsx")
