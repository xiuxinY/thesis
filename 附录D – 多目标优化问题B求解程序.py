...
scaler_CO2_Recovery = MinMaxScaler(
    feature_range=(0, 1)).fit(CO2_Recovery_train)
...
Pareto_Frontier = pd.DataFrame(population.ObjV, columns=[
                               'CO2 Purity', 'CO2 Recovery'])
PF_Phen = pd.DataFrame(population.Phen, columns=[
                       't_AD (s)', 't_CD (s)', 't_BD (s)', 't_VU (s)', 't_FP (s)', 'P_CD (Pa)', 'Ffeed (SLPM)'])
...
plt.scatter(Pareto_Frontier['CO2 Purity'],
            Pareto_Frontier['CO2 Recovery'], alpha=0.5, label='Pareto Frontier')
plt.legend()
